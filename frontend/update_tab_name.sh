...
if [[ -z "$REACT_APP_TAB_NAME" ]]; then
    echo -e "${YELLOW}⚠️  REACT_APP_TAB_NAME não definida, usando valor padrão: Alvo Aguia${NC}"
    TAB_NAME="Alvo Aguia"
else
...
# Substitui todas as ocorrências de "Alvo Aguia" pelo novo nome
sed -i "s/Alvo Aguia/$TAB_NAME/g" public/index.html
...
if [[ -f "public/manifest.json" ]]; then
    sed -i "s/Alvo Aguia/$TAB_NAME/g" public/manifest.json
