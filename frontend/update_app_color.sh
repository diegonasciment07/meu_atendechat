...
if [[ -z "$REACT_APP_COLOR" ]]; then
    echo -e "${YELLOW}⚠️  REACT_APP_COLOR não definida, usando valor padrão: $BASE_COLOR${NC}"
...
if [[ ! "$APP_COLOR" =~ ^#[0-9A-Fa-f]{6}$ ]]; then
    echo -e "${RED}❌ Erro: Cor inválida '$APP_COLOR'. Use formato hexadecimal (ex: #4169e1)${NC}"
    exit 1
fi
...
for file in $js_files; do
    if [[ -f "$file" ]]; then
        ...
        if grep -q "$BASE_COLOR" "$file"; then
            echo -e "${YELLOW}📝 Processando: $file${NC}"
            
            # Conta quantas ocorrências existem no arquivo
            old_count=$(grep -o "$BASE_COLOR" "$file" | wc -l)
            
            # Faz o replace no arquivo
            sed -i "s/$BASE_COLOR/$APP_COLOR/g" "$file"
...
if [[ $files_processed -gt 0 ]]; then
    ...
    echo -e "${YELLOW}⚠️  Nenhum arquivo com a cor $BASE_COLOR foi encontrado${NC}"
fi

# Verifica se ainda existem ocorrências da cor antiga em qualquer arquivo
remaining_old=$(find src -name "*.js" -o -name "*.jsx" -type f -exec grep -l "$BASE_COLOR" {} \; 2>/dev/null)
if [[ -n "$remaining_old" ]]; then
    echo -e "${YELLOW}⚠️  Aviso: Ainda existem ocorrências da cor antiga $BASE_COLOR nos seguintes arquivos:${NC}"
    echo "$remaining_old"
else
