...
exports.createContactListFromTagAndContactList = exports.createContactListFromTag = exports.deleteMedia = exports.mediaUpload = exports.findList = exports.remove = exports.restart = exports.cancel = exports.update = exports.show = exports.store = exports.index = void 0;
...
async function createContactListFromTag(tagId, companyId, campanhaNome) {
    ...
    try {
        ...
        const selectedContacts = await Contact_1.default.findAll({ where: { id: contactIds } });
...
exports.createContactListFromTag = createContactListFromTag;
async function createContactListFromTagAndContactList(tagId, contactListId, companyId, campanhaNome) {
    const currentDate = new Date();
    const formattedDate = currentDate.toISOString();
    try {
        const ticketTags = await TicketTag_1.default.findAll({ where: { tagId } });
        ...
        const tickets = await Ticket_1.default.findAll({ where: { id: ticketIds } });
        const contactIds = tickets.map((ticket) => ticket.contactId);
        const selectedContactListItems = await ContactListItem_1.default.findAll({ where: { contactListId } });
        const ticketContacts = await Contact_1.default.findAll({ where: { id: contactIds } });
        ...
        await ContactListItem_1.default.bulkCreate(contactLists);
        return id;
    }
    catch (error) {
        console.error('Error creating contact list:', error);
...
exports.createContactListFromTagAndContactList = createContactListFromTagAndContactList;
