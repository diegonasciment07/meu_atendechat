...
const flowbuilderIntegration = async (msg, wbot, companyId, queueIntegration, ticket, contact, isFirstMsg, isTranfered) => {
    ...
    */
    ...
    logger_1.logger.info(listPhrase.filter(item => item.phrase.toLowerCase()));
...
const handleMessage = async (msg, wbot, companyId) => {
    ...
    try {
        ...
        if (!msg.key.fromMe &&
            ...
            await (0, exports.handleMessageIntegration)(msg, wbot, integrations, ticket, companyId, isMenu);
            return;
        }
        //openai na fila
        if (!isGroup &&
